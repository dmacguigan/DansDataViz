...
for i in range(1968, 2024): 
  ...
  perc_returning = round(returning / count * 100)
...
perc_returning_list = [] # list to save percent of returning roster year by year
...
for i in range(1968, 2024): 
  ...
  perc_returning_list.append(perc_returning)

  prev_roster_dict = roster_dict # do this to avoid extra data pull requests
```
:::

So now that we have a list with our data values, let's make some figure!
